...
exports.createSauce = (req, res, next) => {
  const saucesObject = JSON.parse(req.body.sauce); // Extraire l'objet json en objet JS
...
exports.modifySauce = (req, res, next) => {
  ...
  const saucesObject = req.file
    ? {
        ...
        ...JSON.parse(req.body.sauce), // on récupère la chaîne de caractère puis on la parse en objet JS
        imageUrl: `${req.protocol}://${req.get("host")}/images/${
          // puis on modifie l'image URL
          req.file.filename
        }`,
      }
    : { ...req.body }; // et Sinon on prend le corps de la reqûete
  Sauces.updateOne(
    { _id: req.params.id },
